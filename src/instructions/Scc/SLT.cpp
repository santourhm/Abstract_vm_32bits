...
void SLT::execute(VMState * vms) 
{

    try
    {
        if (operands.size() != 1 ) {
            throw std::runtime_error(" : SLT requies 1 operand but got " + std::to_string(operands.size()));
        }
        ...
        if(reg == nullptr)
        {
            throw std::runtime_error(" : SLT requires a Rm Operand" );
            return;
        }
