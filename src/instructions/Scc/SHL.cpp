...
void SHL::execute(VMState * vms) 
{

    try
    {
        if (operands.size() != 1 ) {
            throw std::runtime_error(" : SHL requies 1 operand but got " + std::to_string(operands.size()));
        }
        ...
        if(reg == nullptr)
        {
            throw std::runtime_error(" : SHL requires a Rm Operand" );
            return;
        }
