...
void SGE::execute(VMState * vms) 
{

    try
    {
        if (operands.size() != 1 ) {
            throw std::runtime_error(" : SGE requires 1 operand but got " + std::to_string(operands.size()));
        }
        ...
        if(reg == nullptr)
        {
            throw std::runtime_error(" : SGE requires a Rm Operand" );
            return;
        }
